 ...
 <div style="align-content: center;width: 94%;margin-left: 3%;background-color: white;">
     <div class="line-part">
         ...
         <div class="top">
            ...
            <span class="Wdate" onclick="WdatePicker({lang:'zh-cn',onpicked: function (startDate) {getDate(startDate.cal.getDateStr());}})" value="<?php e($today);?>"><?php e($today);?></span>
            ...
            <select class="select"  id="store-type" name="adminRole" size="1" onchange="getStore()" style="width: auto;text-align: center;line-height: 24px;">
                            ...
                            <?php if(isset($store_data_a)){foreach($store_data_a as $data){?>
                            <option value="<?php e($data['StoreID']);?>" style="text-align:center;"><?php e($data["StoreName"]);?></option>
                            <?php }}?>
            </select>
         </div>
        <div class="data-picker">
            ...
            <div class="ul" style="margin-left: 88px;">
                ...
                ?>
                <div  class="date1 li "> <?php e($timeData['午餐'][0]);?>-<?php e($timeData['午餐'][1]);?></div>
                <div class="date2 li"> <?php e($timeData['晚餐'][0]);?>-<?php e($timeData['晚餐'][1]);?> </div>
                <div  class="date3 li"> <?php e($timeData['夜宵'][0]);?>-<?php e($timeData['夜宵'][1]);?> </div>
            </div>
        </div>
    </div>
 </div>
         ...
         <div class="content">
             <div class="items">
                 <div class="left">
                     ...
                     <?php if(isset($box_data)){foreach($box_data as $data){?>
                     <div class="left-info">
                         <span><?php e($data['BoxName']);?></span><span>/<?php e($data['BoxNumber']);?></span>
                     </div>
                     <?php }}?>
                 </div>
                 ...
                 <div class="right">
                     ...
                     <div class="right-info">
                         ...
                         ?>
                         <div class="<?php if($midnight==1){?>box2<?php }elseif($midnight==0){ ?>box1<?php }?> box" onclick="<?php if($midnight==0){?>select(this,<?php e($data['BoxId']);?>,3)<?php }elseif($midnight==1){ ?>booked()<?php }?>"><span><?php if($midnight==1){?>预约<?php }elseif($midnight==0){ ?>空闲<?php }?></span></div>
                         <div class="<?php if($dinner==1){?>box2<?php }elseif($dinner==0){ ?>box1<?php }?> box" onclick="<?php if($dinner==0){?>select(this,<?php e($data['BoxId']);?>,2)<?php }elseif($dinner==1){ ?>booked()<?php }?>"><span><?php if($dinner==1){?>预约<?php }elseif($dinner==0){ ?>空闲<?php }?></span></div>
                         <div class="<?php if($lunch==1){?>box2<?php }elseif($lunch==0){ ?>box1<?php }?> box" onclick="<?php if($lunch==0){?>select(this,<?php e($data['BoxId']);?>,1)<?php }elseif($lunch==1){ ?>booked()<?php }?>"><span><?php if($lunch==1){?>预约<?php }elseif($lunch==0){ ?>空闲<?php }?></span></div>
                     </div>
                     <?php }}?>
                 </div>
             </div>
...
<script>
 var storeId = "<?php e($storeId);?>"; 
var date = "<?php e($today);?>";
var time = 0;
var boxId = 0;
//获取门店ID
function getStore(){
